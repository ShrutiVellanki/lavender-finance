      ...
      <div className="space-y-6">
        <div className="flex items-start sm:items-center gap-3 flex-wrap">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-lg sm:text-xl font-semibold tracking-[-0.02em] text-lavenderDawn-text dark:text-lavenderMoon-text truncate">{account.name}</h1>
            <p className="text-xs sm:text-[13px] text-lavenderDawn-muted dark:text-lavenderMoon-muted capitalize">{account.subtype.replace("_", " ")} · {account.type}</p>
          </div>
          <div className="shrink-0">
            <span className={`text-xl sm:text-2xl font-semibold tabular-nums ${account.current_balance >= 0 ? "text-lavenderDawn-foam dark:text-lavenderMoon-foam" : "text-lavenderDawn-love dark:text-lavenderMoon-love"}`}>
