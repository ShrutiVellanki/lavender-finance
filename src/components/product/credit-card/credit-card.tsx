...
const MastercardLogo = () => (
  <svg viewBox="0 0 38 24" className="h-8 w-auto">
    ...
    <path
      d="M19 5.8A10 10 0 0 1 22.8 12 10 10 0 0 1 19 18.2 10 10 0 0 1 15.2 12 10 10 0 0 1 19 5.8z"
      fill="#FF5F00"
    />
  </svg>
);
    ...
    <div className="absolute inset-0 backface-hidden rounded-2xl p-6 flex flex-col justify-between bg-gradient-to-br from-lavenderDawn-iris via-lavenderDawn-pine to-lavenderDawn-foam dark:from-lavenderMoon-iris dark:via-lavenderMoon-pine dark:to-lavenderMoon-foam shadow-2xl overflow-hidden">
      ...
      <div className="relative flex items-end justify-between">
        <div>
          ...
          <p className="text-white font-medium tracking-wide uppercase text-sm truncate max-w-[180px]">
            {data.name || "Full Name"}
          </p>
        </div>
    ...
    >
      ...
      <div className="px-6 space-y-3">
        ...
        <p className="text-white/50 text-[10px] leading-relaxed">
          This card is property of Lavender Finance. Misuse is subject to
          prosecution. If found, please return to the nearest branch.
        </p>
      </div>
...
const CreditCardDisplay: React.FC<CreditCardDisplayProps> = ({ data, flipped }) => (
...
const CardInput: React.FC<InputProps> = ({ label, ...props }) => (
  <div className="flex flex-col gap-1">
    ...
    <input
      {...props}
      className="bg-lavenderDawn-overlay dark:bg-lavenderMoon-overlay border border-lavenderDawn-highlightMed dark:border-lavenderMoon-highlightMed rounded-lg px-3 py-2.5 text-sm text-lavenderDawn-text dark:text-lavenderMoon-text placeholder:text-lavenderDawn-muted dark:placeholder:text-lavenderMoon-muted focus:outline-none focus:ring-2 focus:ring-lavenderDawn-iris/40 dark:focus:ring-lavenderMoon-iris/40 focus:border-lavenderDawn-iris dark:focus:border-lavenderMoon-iris transition-all duration-200 font-mono tracking-wide"
    />
  </div>
);
